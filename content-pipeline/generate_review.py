...
def generate_pinyin_for_word(word):
    """Generate pinyin: Style.TONE, space-separated, base tones only (strip  sandhi)."""
    syllables = pinyin(word, style=Style.TONE)
    flat = [s[0] for s in syllables]
    # Strip  sandhi: force b (base) instead of b (pypinyin's auto-sandhi)
    if BU_CHAR in word:
...
def process_chapter(input_path):
    ...
    print(f"[INFO] Passage length: {len(full_passage)} characters")
    print(f"[INFO] Vocab items: {len(vocab_list)}")
    print(f"[INFO] English items: {len(english_list)}")
    
    if len(vocab_list) != len(english_list):
        print(f"[ERROR] Vocab count ({len(vocab_list)}) doesn't match English count ({len(english_list)}).")
        sys.exit(1)
    
    vocab_list, english_list, not_found = sort_vocab_by_appearance(vocab_list, english_list, full_passage)
    if not_found:
    ...
    for word, meaning in zip(vocab_list, english_list):
        results.append({
            ...
            "context": extract_context_sentence(word, full_passage),
        })
    return chapter_id, chapter_title, results


def write_review_excel(chapter_id, chapter_title, results, output_path):
    wb = Workbook()
    ws = wb.active
    ws.title = "Review"
    ...
    ws["A2"] = "REVIEW: check Pinyin (fix multi-pronunciation errors) and ContextSentence (fix if wrong/missing)."
    ws["A2"].font = Font(italic=True, color="666666")
    ws.merge_cells("A2:D2")
    
    headers = ["Traditional", "English", "Pinyin (auto - review!)", "ContextSentence (auto - review!)"]
    ...
    for i, r in enumerate(results, start=4):
        ...
        if not r["context"]:
            ...
            ws.cell(row=i, column=4).comment = Comment(
                "Word not found in passage. Please add context sentence manually.", "System"
            )
...
def main():
    parser = argparse.ArgumentParser(description="Generate review Excel from raw chapter input.")
    parser.add_argument("--input", required=True, help="Path to chapterXX_raw.xlsx (relative to content-pipeline/)")
    ...
    chapter_id, chapter_title, results = process_chapter(input_path)
    ...
    write_review_excel(chapter_id, chapter_title, results, output_path)
